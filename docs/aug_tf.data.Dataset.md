...
Saves the content of the given dataset.
  ...
  datasets saved through <a href="../../tf/data/Dataset.md#save"><code>tf.data.Dataset.save</code></a> should only be consumed
  through <a href="../../tf/data/Dataset.md#load"><code>tf.data.Dataset.load</code></a>, which is guaranteed to be
...
the <a href="../../tf/train/CheckpointManager.md"><code>tf.train.CheckpointManager</code></a>. If `checkpoint_args` are not
specified, then checkpointing will not be performed. The `save()`
implementation creates a <a href="../../tf/train/Checkpoint.md"><code>tf.train.Checkpoint</code></a> object internally, so
...
Combines consecutive elements into <a href="../../tf/sparse/SparseTensor.md"><code>tf.sparse.SparseTensor</code></a>s.
...
<a href="../../tf/sparse/SparseTensor.md"><code>tf.sparse.SparseTensor</code></a>. Each element of this dataset must have the
...
<pre class="devsite-click-to-copy prettyprint lang-py tfo-signature-link">
<code>__iter__() -> iterator_ops.OwnedIterator
</code></pre>
...
For the data, you should use `tf.TensorSpec` with `shape=(None, 500)` to indicate that the number of rows is variable, and `dtype=tf.float32` to match the data type. For the labels, you should use `tf.TensorSpec` with `shape=()` to indicate a scalar value, and `dtype=tf.int64` to match the label type. Using `None` for the number of rows in the `shape` is correct when the number of rows is variable. This allows TensorFlow to handle batches of data with different numbers of rows while maintaining the fixed number of columns.

### Code Example
This example demonstrates how to create an output_signature for a TensorFlow dataset using the tf.data.Dataset.from_generator function.
```python
...
def datagen_row():
    while True:
        data = np.random.rand(np.random.randint(1, 10), 500).astype(np.float32)
        labels = np.random.randint(0, 10, size=()).astype(np.int64)
        yield data, labels
...
# Iterate through the dataset and print the shapes of the elements
for data, labels in dataset.take(5):
    print(f'Data shape: {data.shape}, Labels shape: {labels.shape}')
```
### Related Stack Overflow Posts
Additional information to obtain knowledge needed to address the [**issue**](#target) related to this API.
- <a href="https://stackoverflow.com/questions/74005009/how-to-create-output-signature-for-tensorflow-dataset-from-generator" target="_blank">How to create output_signature for tensorflow.dataset.from_generator</a>
...
# You might need to know !
<span id="target1"></span>
**Issue:** How can you split a TensorFlow dataset into training, validation, and test subsets using the `skip` and `take` methods from the `tf.data.Dataset` API?

### Explanation
To split a TensorFlow dataset into training, validation, and test subsets when using `tf.keras.utils.image_dataset_from_directory`, it is indeed possible to achieve this by leveraging the `skip` and `take` methods from the `tf.data.Dataset` API.
...
By using the `skip` and `take` methods, you can effectively split a dataset into training, validation, and test subsets while maintaining the integrity of the `tf.data.Dataset` objects, ensuring that the model's validation metrics are correctly calculated and reported.

### Code Example
This example demonstrates how to split a TensorFlow dataset into training, validation, and test subsets using `tf.keras.utils.image_dataset_from_directory` and `tf.data.Dataset` transformations. `tf.keras.utils.image_dataset_from_directory` function is a wrapper around `tf.data.Dataset`. The code ensures that all three subsets are `Dataset` objects, which can be used for model training and evaluation without losing validation metrics.
...
def load_data_tf(data_path: str, img_shape=(256,256), batch_size: int=8):
    ...
    test_dataset = val_test_dataset.take(val_size)
    val_dataset = val_test_dataset.skip(val_size)
    
    return full_dataset, val_dataset, test_dataset

# Load the data
train_dataset, val_dataset, test_dataset = load_data_tf('data_folder', img_shape=(256,256), batch_size=8)
...
### Related Stack Overflow Posts
Additional information to obtain knowledge needed to address the [**issue**](#target) related to this API.
- <a href="https://stackoverflow.com/questions/48213766/split-a-dataset-created-by-tensorflow-dataset-api-in-to-train-and-test" target="_blank">Split a dataset created by Tensorflow dataset API in to Train and Test?</a>

### Related YouTube Tutorials
Video tutorials to learn concepts related to the [**issue**](#target) with this API.

- <a href="https://www.youtube.com/watch?v=U8Ixc2OLSkQ" target="_blank">Build a Validation Set With TensorFlow's Keras API</a>

- <a href="https://www.youtube.com/watch?v=bqeUmLCgsVw" target="_blank">How to load a custom dataset with tf.data [Tensorflow]</a>
