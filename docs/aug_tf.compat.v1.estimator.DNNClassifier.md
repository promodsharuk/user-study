...
A classifier for TensorFlow DNN models. (deprecated) (deprecated)
...
<span id="target"></span>
**Issue:** How can data type mismatches be resolved when testing a TensorFlow text classification model trained with `tf.compat.v1.estimator.DNNClassifier` and served using `export_savedmodel`?

### Explanation
When using `tf.compat.v1.estimator.DNNClassifier` and saving the model with `export_savedmodel`, data type mismatches often arise during testing with `saved_model_cli`, especially with string inputs.

These issues stem from inconsistencies in how string features are handled. Using `tf.VarLenFeature` may cause errors when converting `SparseTensor` to `Tensor`, while switching to `tf.placeholder` can resolve saving but still lead to testing errors involving string and bytes mismatches.

To fix this, ensure the input types match what’s defined in the `serving_input_receiver_fn`. Provide input as strings (not bytes) during testing and verify the placeholders align with the serving format. Consistency here prevents type conversion errors during model serving and testing.

### Code Example
The code defines and trains a TensorFlow text classification model using the `tf.compat.v1.estimator.DNNClassifier` API, which processes text features through embedding columns and exports the model for serving.
```python
...
class TextClassifierModel:
    ...
    def _build_estimator(self):
        feature_columns = [
            tf.feature_column.embedding_column(
                tf.feature_column.categorical_column_with_hash_bucket('Headline', hash_bucket_size=1000),
                dimension=16),
            tf.feature_column.embedding_column(
                tf.feature_column.categorical_column_with_hash_bucket('Description', hash_bucket_size=1000),
                dimension=16)
        ]
    ...
    def serving_input_receiver_fn(self):
        feature_spec = {
            'Headline': tf.placeholder(dtype=tf.string, shape=[None], name='Headline'),
            'Description': tf.placeholder(dtype=tf.string, shape=[None], name='Description')
        }
...
train_input_fn = tf.estimator.inputs.pandas_input_fn(
    x=pd.DataFrame({
        'Headline': ['Help me', 'What is going on'],
        'Description': ['I need assistance', 'Can someone explain?']
    }),
    y=pd.Series([0, 1]),
    batch_size=2,
    num_epochs=None,
...
classifier = TextClassifierModel(model_dir)
classifier.train(train_input_fn, steps=100)
classifier.export_model(export_dir)

```
### Related Stack Overflow Posts
Additional information to obtain knowledge needed to address the [**issue**](#target) related to this API.

- <a href="https://stackoverflow.com/questions/52079029/tensorflow-estimator-dnnclassifier-export-savedmodel-caused-valueerror-invali" target="_blank">TensorFlow estimator.DNNClassifier: export_savedmodel caused "ValueError: Invalid feature"</a>
